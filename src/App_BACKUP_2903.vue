...
<template>
  ...
  <header :class="{ dark: darkMode }">
    <nav
      id="topnav"
      :class="{ 'py-5 drop-shadow-[0px_0px_1px_#454545]': !bigNavBar }"
    ...
    >
      <!-- menu button -->
      <div
        class="grid grid-cols-2 gap-2 cursor-pointer h-fit w-fit justify-self-start group"
        @click="toggleSideBar"
      >
      ...
      <div class="relative flex items-center justify-center text-center">
        <div
          class="z-10 px-3 text-base font-light text-white uppercase bg-dark font-inter tracking-[5.7px]"
        >
          Welcome
        </div>
        <!-- Line -->
      ...
      <!-- Lightbulb button -->
      <div
        class="w-[30px] h-[30px] relative cursor-pointer text-black dark:text-white"
        @click="toggleDark"
      >
        <!-- Lightbulb SVG -->
  ...
  <main :class="{ dark: darkMode }">
    <!-- side nav -->
<<<<<<< HEAD
    <div class="fixed min-w-[64px] items-center flex h-full z-30">
      <div class="gap-2 text-white uppercase font-inter text-md">
        <ul class="flex flex-col gap-2">
          <li
            v-for="(link, index) in links"
            ref="links"
            class="p-2 mx-5 transition-all duration-300 ease-in-out rounded-lg shadow-sm shadow-white"
            :key="'link-' + index"
            :class="[
              sideBarOpen
          ...
          >
            <a :href="link.link">{{ link.text }}{{ index * 300 }}</a>
=======
    <div
      class="fixed min-w-[64px] items-center flex h-full z-30"
      ...
      <div class="gap-2 px-5 text-white uppercase font-inter text-md">
        <ul class="flex flex-col gap-2">
          <li class="p-2 rounded-lg shadow-sm shadow-white">
            <RouterLink
              to="/projects"
              @click="scrollToSection('#projects', 'projects')"
              >Projects</RouterLink
            >
          </li>

          <li class="p-2 rounded-lg shadow-sm shadow-white">
            <RouterLink to="/projects">Contact</RouterLink>
          </li>
          <li class="p-2 rounded-lg shadow-sm shadow-white">
            <RouterLink to="/projects">Contact</RouterLink>
          </li>
          <li class="p-2 rounded-lg shadow-sm shadow-white">
            <RouterLink to="/projects">Contact</RouterLink>
>>>>>>> 92dec86bd3689f323d6c92a1e09a24bcea0f22af
          </li>
        </ul>
      </div>
    </div>
...
=======
    ...
    // Listen to route changes and scroll to the corresponding section
    router.beforeEach((to, from, next) => {
      this.scrollToSection(to.hash, "projects");
      next();
    });
>>>>>>> 92dec86bd3689f323d6c92a1e09a24bcea0f22af
  },
  ...
  methods: {
    scrollToSection(path: string, routeName: string) {
      console.log(path);
      const element = <HTMLElement>document.querySelector(path);
      if (element) {
        window.scrollTo({
...
<<<<<<< HEAD
      ...
      for (let i = 0; i < 4; i++) {
        const navdots = <HTMLElement[]>this.$refs.navdots;
        const links = <HTMLElement[]>this.$refs.links;

        navdots[i].style.position = "absolute";
        navdots[0].style.bottom = String(
...
<<<<<<< HEAD
      const links = <HTMLElement[]>this.$refs.links;
      const link = <HTMLElement>links[0];

      // Check if the element exists and is not undefined
      if (link) {
  ...
  computed: {
    calculateDelay() {
      return (index: number) => `delay-[${index * 100}ms]`;
    },
  },
};
</script>
